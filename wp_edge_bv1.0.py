...
def read_plc_nodes(plc, plc_name):
    ...
    try:
        # ✅ BULK READ: Prepare all variables for bulk reading
        read_list = []
        var_names = []
        ...
        for var in PYADS_VARIABLES:
            ...
            if var_type:
                read_list.append((var_name, var_type))
                var_names.append(var_name)
            else:
                print(f"Warning: Unknown data type '{var['type']}' for {var_name}")
        
        # ✅ BULK READ: Read all variables in one operation
        values = plc.read_list_by_name(read_list)
        
        # ✅ Process results and store in data dictionary
        ...
        print(f"❌ Error in bulk read operation: {e}")
        # Fallback to individual reads if bulk fails
        print("🔄 Falling back to individual reads...")
        for var in PYADS_VARIABLES:
            var_name = var['name']
            var_type = type_mapping.get(var['type'])
            try:
    ...
    for i in range(1, 111):  # Changed to 111 to include queue 110
        ...
        lift_no = data.get(f"Request_Queue_Status_{i}_Lift_No")  # Consistent naming
        ...
        queue_data_list.append({
            ...
            "Lift_No": lift_no if lift_no and lift_no > 0 else "NULL",  # Added null check
        })
...
def create_parking_map_from_file(plc, plc_name, current_date, current_time, parking_map_file, queue_data_list):
    try:
        ...
        for node_info in parking_map_nodes:
            ...
            if data_type:
                read_list.append((var_name, data_type))
            else:
                print(f"Warning: Unknown data type '{data_type_str}' for {var_name}, skipping.")

        # ✅ Bulk read using pyads.read_list_by_name
        values = plc.read_list_by_name(read_list)

        # ✅ Filter out zero/null values
        non_zero_token_values = [val if val is not None else 0 for val in values]
        non_zero_token_values = [v for v in non_zero_token_values if v != 0]
